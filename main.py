...
class BusAirQualityApp:
    ...
    def _build_ui(self):
        ...
        self.live_co2_var  = tk.StringVar(value="CO2: --- ppm")
    ...
    def _build_measurement_tab(self, parent, title, color_accent, bus_type):
        ...
        # 상태 표시
        status_frame = tk.Frame(parent, bg="#2c3e50", pady=10)
        ...
        card_defs = [
            ("CO2(ppm)",  co2_var,  "#e74c3c"),
            ("온도(C)",   temp_var, "#3498db"),
            ("습도(%)",   humi_var, "#2ecc71"),
            ("등급",      lvl_var,  "#f1c40f"),
        ]

        for label, var, clr in card_defs:
        ...
        sb_y = tk.Scrollbar(log_frame, command=log_box.yview)
        sb_x = tk.Scrollbar(log_frame, orient="horizontal",
                             command=log_box.xview)
        log_box.configure(yscrollcommand=sb_y.set,
        ...
        def _loop():
            while True:
                ...
                if co2 is not None:
                    ...
                    # ★ f-string 안에 % 포맷 제거 → 변수로 분리
                    co2_str  = "CO2: " + str(co2) + " ppm"
                    temp_str = "온도: " + str(temp) + "C"
                    humi_str = "습도: " + str(humi) + "%"
                    lvl_str  = "등급: " + lvl
                    time_str = now
                    self.live_co2_var.set(co2_str)
                    self.live_temp_var.set(temp_str)
                    self.live_humi_var.set(humi_str)
                    self.live_lvl_var.set(lvl_str)
                    self.live_time_var.set(time_str)
                time.sleep(2)
    ...
    def _start_measurement(self, bus_type):
        if bus_type == "gas":
            ...
            self._log(self.gas_log_box, "▶ 측정 시작", color="start")
            threading.Thread(
            ...
            self._log(self.hydro_log_box, "▶ 측정 시작", color="start")
            threading.Thread(
    ...
    def _measure_loop(self, bus_type):
        ...
        while True:
            ...
            if co2 is not None:
                ...
                # ★ 핵심 수정: f-string 분리로 SyntaxError 방지
                log_line = (
                    "[" + now + "] "
                    "#" + str(count).zfill(3) + " | "
                    "CO2: " + str(co2) + " ppm | "
                ...
                if bus_type == "gas":
                    ...
                    self.root.after(0, lambda r=record, c=count:
                        self._update_live_cards("gas", r))
                    self.root.after(0, lambda line=log_line:
                        self._log(self.gas_log_box, line))
                else:
                    self.hydro_current_readings.append(record)
                    self.root.after(0, lambda r=record:
                        self._update_live_cards("hydro", r))
                    self.root.after(0, lambda line=log_line:
                        self._log(self.hydro_log_box, line))
    ...
    # 카드 업데이트
    # ──────────────────────────────────────────────────────
    def _update_live_cards(self, bus_type, record):
        co2  = record["co2"]
        ...
        if bus_type == "gas":
            self.gas_co2_var.set(str(co2))
            self.gas_temp_var.set(str(temp))
            self.gas_humi_var.set(str(humi))
            self.gas_lvl_var.set(lvl)
        else:
            self.hydro_co2_var.set(str(co2))
            self.hydro_temp_var.set(str(temp))
            self.hydro_humi_var.set(str(humi))
    ...
    def _stop_measurement(self, bus_type):
        ...
        if not readings:
            self._log(log_box, "경고: 측정된 데이터가 없습니다.", color="warn")
            return
        ...
        session = {
            ...
            "stats": {
                "co2": {
                    ...
                    "표준편차": round(
                        statistics.stdev(co2_values)
                        if len(co2_values) > 1 else 0, 2)
                },
                "temperature": {
        ...
        session_var.set("측정 세션: " + str(len(sessions)) + "회 완료")
        ...
        sep  = "-" * 52
        # ★ 핵심 수정: f-string 대신 문자열 연결로 변경
        msg1 = (
            "■ 세션 #" + str(session["session_no"]) +
            " 종료  (" + str(session["count"]) + "회 측정 | " +
        ...
        self._log(log_box, sep,  color="sep")
        self._log(log_box, msg1, color="stop")
        self._log(log_box, msg2, color="result")
        self._log(log_box, sep,  color="sep")
    ...
    def _clear_sessions(self, bus_type):
        answer = messagebox.askyesno("초기화 확인",
                                     "모든 측정 데이터를 삭제하시겠습니까?")
        if not answer:
            return
    ...
    def _update_compare_tab(self):
        ...
        # ★ strftime 결과를 변수로 분리하여 f-string 충돌 방지
        now_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        lines = []
        lines.append("=" * 58)
        lines.append("  비교 결과 요약")
        lines.append("  생성 시각: " + now_str)   # ← 수정된 부분
        lines.append("=" * 58)

        # 가스버스 요약
        lines.append("\n가스버스")
        lines.append("  측정 세션: " + str(len(gas_s)) + "회")
        ...
        if gas_s:
            ...
            lines.append("  전체 측정 횟수: " + str(len(all_co2)) + "회")
            lines.append("  CO2 평균: " + str(g_avg) + " ppm   [" + lvl + "]")
            lines.append("  CO2 최대: " + str(g_max) + " ppm")
            lines.append("  CO2 최소: " + str(g_min) + " ppm")
            lines.append("\n  세션별 상세:")
            for s in gas_s:
                avg = s["stats"]["co2"]["평균"]
                mx  = s["stats"]["co2"]["최대"]
        ...
        # 수소전기버스 요약
        lines.append("\n수소전기버스")
        lines.append("  측정 세션: " + str(len(hydro_s)) + "회")
        ...
        if hydro_s:
            ...
            lines.append("  전체 측정 횟수: " + str(len(all_co2_h)) + "회")
            lines.append("  CO2 평균: " + str(h_avg) + " ppm   [" + lvl_h + "]")
            lines.append("  CO2 최대: " + str(h_max) + " ppm")
            lines.append("  CO2 최소: " + str(h_min) + " ppm")
            lines.append("\n  세션별 상세:")
            for s in hydro_s:
                avg = s["stats"]["co2"]["평균"]
                mx  = s["stats"]["co2"]["최대"]
        ...
        if g_avg is not None and h_avg is not None:
            ...
            lines.append("  가스버스 평균 CO2     : " + str(g_avg) + " ppm")
            lines.append("  수소전기버스 평균 CO2 : " + str(h_avg) + " ppm")
            lines.append("  차이 (가스 - 수소)    : " + str(diff)  + " ppm")
            lines.append("")
            if diff > 50:
                lines.append("  ✅ 수소전기버스가 가스버스보다")
                lines.append("     CO2를 " + str(diff) + " ppm 더 낮게 유지합니다!")
                lines.append("  수소전기버스가 확연히 친환경적입니다.")
            elif diff > 10:
                lines.append("  ✅ 수소전기버스가 " + str(diff) + " ppm 더 낮습니다.")
                lines.append("  수소전기버스가 친환경적입니다.")
            elif diff > 0:
                lines.append("  수소전기버스가 약간 낮습니다. (" + str(diff) + " ppm)")
                lines.append("  추가 측정을 권장합니다.")
            else:
                lines.append("  이번 측정에서는 차이가 미미합니다.")
    ...
    def _save_results(self):
        ...
        filename = "result_" + filename_time + ".json"
    ...
    def _log(self, log_box, text, header=False, color="normal"):
        ...
        log_box.tag_configure(
            tag,
            foreground=color_map.get(color, "#58d68d"),
            font=("Consolas", 9, "bold" if header else "normal")
        )
